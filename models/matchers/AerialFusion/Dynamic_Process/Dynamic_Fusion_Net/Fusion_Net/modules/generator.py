...
class Generator(nn.Module):
    ...
    def forward(self, ir_img, vi_img, SG_ir = None, SG_vi = None):
        ...
        v_layer4, x_layer1,attention_layer4= self.brightA1(ir_layer3, ir_layer2)                   # 0  1  2
        v_layer3, x_layer2, attention_layer3 = self.brightA2(ir_layer2, ir_layer1)                 # 34 56 78
...
class GFusion(nn.Module):
    ...
    def forward(self, ir_img, ir_layer1, ir_layer11, ir_layer2, ir_layer21, ir_layer3, attention_layer4, v_layer4,
        ...
        if SG_ir is not None and SG_vi is not None:
            pre_fusion_ir, pre_fusion_vi = ir_img + SG_ir, vi_img + SG_vi
        else:
            pre_fusion_ir, pre_fusion_vi = ir_img, vi_img

        ir_features = torch.cat([pre_fusion_ir, ir_layer1, ir_layer11, ir_layer2, ir_layer21, ir_layer3, attention_layer4, v_layer4,
                                 attention_layer3, v_layer3], dim=1)                                                     # 红外特征整合
        vi_features = torch.cat([pre_fusion_vi, vi_layer1, torch.cat([vi_img, vi_layer1],dim=1), vi_layer2, torch.cat([vi_img, vi_layer1, vi_layer2], dim=1),
                                 vi_layer3, seman_layer2, seman_layer1], dim=1)                                          # 可见光特征整合
