...
def sample_homography_corners(
    ...
    if n_angles > 0 and difficulty > 0:
        angles = np.linspace(-max_angle * difficulty, max_angle * difficulty, n_angles)           # 角度数组（起始、结束、间隔）
        rng.shuffle(angles)
        rng.shuffle(angles)                                                                       # 随机角度
...
def warp_points(points, homography, inverse=True):
    ...
    H_inv = np.transpose(np.linalg.inv(H) if inverse else H)                                   # H的逆
