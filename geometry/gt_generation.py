...
def gt_matches_from_homography(kp0, kp1, H, pos_th = 3, neg_th = 6, **kw):
    """
    构建 基于单应变换特征点匹配 的 Ground_Truth
    (kp0, kp1, H, 位置置信阈值（像素数）, 相邻置信阈值（像素数）)
    """
    if kp0.shape[1] == 0 or kp1.shape[1] == 0:
        b_size, n_kp0 = kp0.shape[:2]
        n_kp1 = kp1.shape[1]
    ...
    kp0_1 = warp_points_torch(kp0, H, inverse = False)                   # 正常变换, kp0-->kp1
    kp1_0 = warp_points_torch(kp1, H, inverse = True)                    # 逆变换,   kp1-->kp0

    dist0 = torch.sum((kp0_1.unsqueeze(-2) - kp1.unsqueeze(-3)) ** 2, -1)            # 计算 kp0_1--kp1 距离
    dist1 = torch.sum((kp0.unsqueeze(-2) - kp1_0.unsqueeze(-3)) ** 2, -1)            # 计算 kp0--kp1_0 距离
    dist = torch.max(dist0, dist1)                                                   # 取距离最大值
    ...
    ismin0.scatter_(-1, min0.unsqueeze(-1), value=1)
    ismin1.scatter_(-2, min1.unsqueeze(-2), value=1)
    positive = ismin0 & ismin1 & (dist < pos_th**2)                                  # 筛选 满足条件 特征点索引
